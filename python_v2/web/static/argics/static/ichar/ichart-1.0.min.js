(function(c) {
    ...
    q = {
        easeIn: function(a, b, c, h) {
            return c * (a /= h) * a * a + b
        },
        easeOut: function(a, b, c, h) {
            return c * ((a = a / h - 1) * a * a + 1) + b
        },
        easeInOut: function(a, b, c, h) {
            return 1 > (a /= h / 2) ? c / 2 * a * a * a + b: c / 2 * ((a -= 2) * a * a + 2) + b
        }
    },
    m = function(a) {
        var c = !1,
        ...
        p.merge = function(a, h, b) {
            if (a && p.isObject(h)) {
                for (var c in h) p.isDefined(h[c]) && (p.isObject(h[c]) ? p.isObject(a[c]) ? p.merge(a[c], h[c]) : a[c] = p.clone(h[c], !0) : a[c] = h[c]);
        ...
        p.clone = function(a, h, b) {
            var c = {};
            if (p.isArray(a) && p.isObject(h)) for (var d = 0; d < a.length; d++) c[a[d]] = b && p.isObject(h[a[d]]) ? p.clone(h[a[d]], b) : h[a[d]];
            else if (p.isObject(a)) for (d in a) c[d] = h && p.isObject(a[d]) && !a[d].ICHARTJS_OBJECT ? p.clone(a[d], h) : a[d];
            return c
        };
        ...
        P = function(a, h, b, c) {
            ...
            if (0 == v) d = [0, 0, f];
                ...
                var g;
                d == f ? g = (s - e) / v: s == f ? g = (e - d) / v + 2 : e == f && (g = (d - s) / v + 4);
                g *= 60;
                0 > g && (g += 360);
                d = [g, v / f, f]
            }
            d[1] -= 0 != c ? c || 0.05 : c;
            a ? (d[2] -= O(d[2], b), d[1] = p.upTo(d[1], 1), d[2] = p.lowTo(d[2], 0)) : (d[2] += O(1 - d[2], b), d[1] = p.lowTo(d[1], 0), d[2] = p.upTo(d[2], 1));
            c = h[3];
            a = h = void 0;
            p.isArray(d) && (a = c, c = d[1], h = d[2], d = d[0]);
            var w, j, i, b = E(d / 60) % 6,
            e = d / 60 - b;
            d = h * (1 - c);
            s = h * (1 - c * e);
            c = h * (1 - c * (1 - e));
        ...
        p.apply(p, {
            ...
            distanceP2P: function(a, h, b, d) {
                return R((b - a) * (b - a) + (d - h) * (d - h))
            },
            atan2Radian: function(a, h, b, d) {
                if (a == b) return d > h ? y / 2 : 3 * y / 2;
                if (h == d) return b > a ? 0 : y;
                var c = p.quadrant(a, h, b, d),
                a = n(H((h - d) / (a - b)));
                return c ? (3 == c ? A: y) + (2 == c ? a: -a) : a
            },
            ...
            visible: function(a, h, b) {
                ...
                var d = p.quadrantd(a),
                c = p.quadrantd(h);
                if ((2 == d || 3 == d) && (2 == c || 3 == c) && h - a < y) return [];
                a = p.toPI2(a);
                h = p.toPI2(h);
                h <= a && (h += A);
                if (a > y) a = A;
        ...
        p.Event = {
            ...
            fix: function(h) {
                ...
                if ("undefined" == typeof h.offsetX) {
                    ...
                    for (var c = d = 0,
                    s = h.target; s != document.body && s;) d += s.offsetLeft,
                    c += s.offsetTop,
                    s = s.offsetParent;
                    b.offsetX = b.pageX - d;
                    b.offsetY = b.pageY - c
    ...
    Array.prototype.each = function(a, b) {
        ...
        h, c = 0; c < d && !(h = b ? a.call(b, this[c], c) : a(this[c], c), "boolean" === typeof h && !h); c++);
        return this
    };
    Array.prototype.eachAll = function(a, b) {
        this.each(function(d, h) {
            return m.isArray(d) ? d.eachAll(a, b) : b ? a.call(b, d, h) : a(d, h)
        },
        b)
    };
    Array.prototype.sor = function(a) {
        for (var b = this.length - 1,
        d, h = 0; h < b; h++) for (var c = b; c > h; c--) if (a ? !a(this[c], this[c - 1]) : this[c] < this[c - 1]) d = this[c],
        this[c] = this[c - 1],
        this[c - 1] = d
    };
    c.iChart = m;
    ...
    c.Element.prototype = {
        ...
        fireEvent: function(a, b, d) {
            ...
            if (1 == c) return this.events[b][0].apply(a, d);
            for (var f = !0,
            g = 0; g < c; g++) this.events[b][g].apply(a, d) || (f = !1);
            return f
        },
        on: function(a, b) {
            if (c.isString(a)) {
                if (!this.events[a]) throw Error("[" + this.type + "] invalid event:'" + a + "'");
                this.events[a].push(b)
            } else c.isArray(a) && a.each(function(a) {
                this.on(a, b)
            },
            this);
            return this
        ...
        push: function(a, b) {
            for (var d = a.split("."), c = d.length - 1, f = this.options, g = 0; g < c; g++) f[d[g]] || (f[d[g]] = {}),
            f = f[d[g]];
            return f[d[c]] = b
        },
        get: function(a) {
    ...
    c.Painter = c.extend(c.Element, {
        ...
        applyGradient: function(a, b, d, c) {
            var f = this._();
            f.get("gradient") && f.get("f_color") && (f.push("f_color", f.T.gradient(a || f.x || 0, b || f.y || 0, d || f.get(f.W), c || f.get(f.H), [f.get("dark_color"), f.get("light_color")], f.get("gradient_mode"))), f.push("light_color", f.T.gradient(a || f.x || 0, b || f.y || 0, d || f.get(f.W), c || f.get(f.H), [f.get("background_color"), f.get("light_color")], f.get("gradient_mode"))), f.push("f_color_", f.get("f_color")))
        },
        draw: function(a, b) {
    ...
    c.Html = c.extend(c.Element, {
        ...
        applyStyle: function() {
            for (var a = this.get("style").split(";"), b, d = 0; d < a.length; d++) b = a[d].split(":"),
    ...
    c.Tip = c.extend(c.Html, {
        ...
        follow: function(a, b) {
            ...
            if (d.get("invokeOffsetDynamic")) {
                if (b.hit) { (c.isString(b.text) || c.isNumber(b.text)) && d.text(b.name, b.value, b.text, b.i, d);
                    var e = d.get("invokeOffset")(d.width(), d.height(), b);
                    d.position(e.top, e.left)
                }
            } else "follow" != d.get("showType") && c.isFunction(d.get("invokeOffset")) ? (e = d.get("invokeOffset")(d.width(), d.height(), b), d.position(e.top, e.left)) : d.position(a.y - 1.1 * d.height() - 2, a.x + 2)
        },
        ...
        doAction: function(a) {
            ...
            function(b, c, e) {
                a.T.variable.event.mouseover && setTimeout(function() {
                    a.T.variable.event.mouseover && a.follow(c, e)
                },
                a.get("delay"))
            })
        },
    ...
    c.Text = c.extend(c.Component, {
        ...
        a.Chart = a.extend(a.Painter, {
            ...
            commonDraw: function(b, c) {
                b.redraw || (a.Assert.isTrue(b.Rendered, b.type + " has not rendered"), a.Assert.isTrue(b.initialization, b.type + " Failed to initialize"), a.Assert.isTrue(0 < b.data.length, b.type + "'s data is empty"), b.doSort(), b.oneways.eachAll(function(a) {
                    a.draw()
                }));
            ...
            initialize: function() {
                ...
                c = b.get("data"),
                d = b.get("render");
                b.Combination ? (a.apply(b.options, a.clone([b.W, b.H, "padding", "border", "client_height", "client_width", "minDistance", "maxDistance", "minstr", "centerx", "centery", "l_originx", "r_originx", "t_originy", "b_originy"], b.root.options, !0)), b.width = b.get(b.W), b.height = b.get(b.H), b.shell = b.root.shell, b.Rendered = !0) : b.Rendered || d && b.create(b, a(d));
                0 < c.length && b.Rendered && !b.initialization && (u.call(b, c, b), b.data = c, b.doConfig(), b.initialization = !0)
            },
            eventOff: function() {
                this.stopEvent = !0
            },
    ...
    c.Scale = c.extend(c.Component, {
        ...
        doConfig: function() {
            ...
            a.isH = "h" == a.get("which");
            a.isH ? (l == a.O ? i = -k: l == a.C ? (i = -o, e = o) : e = k, r == a.O ? (m = -t - h[0], u = a.B) : (m = t + h[2], u = a.O), r = a.C) : (l == a.L ? f = -k: l == a.C ? (f = -o, b = o) : b = k, u = "middle", r == a.R) ? (r = a.L, q = t + h[1]) : (r = a.R, q = -t - h[3]);
    ...
    c.Line = c.extend(c.Chart, {
        ...
        doConfig: function() {
            ...
            var d = a.coo.get("valid_width"),
            e = a.coo.get("valid_height"),
            f = d / (a.get("maxItemSize") - 1),
            g = a.get("point_space");
            a.get("proportional_spacing") && (g && g < f ? d = (a.get("maxItemSize") - 1) * g: a.push("point_space", f));
            a.push("sub_option.width", d);
            a.push("sub_option.height", e);
            a.pushIf("sub_option.keep_with_coordinate", b);
            a.push("sub_option.originx", a.coo.get("x_start") + (a.coo.get("valid_width") - d) / 2);
