...
(function($) {
	...
	$.ui.mouseInteraction = function(el,o) {
		...
		/* modified by liusp begin 2. */
		//if($.browser.msie) $(this.element).attr('unselectable', 'on');
		if(_IE) $(this.element).attr('unselectable', 'on');
		/* modified by liusp end 2. */		
	}
	...
	$.extend($.ui.mouseInteraction.prototype, {
		...
		drag: function(e) {
			...
			/* modified by liusp begin 3. */
			//if (!this.opos || ($.browser.msie && !e.button && parseFloat($.browser.version)<9)) return this.stop.apply(this, [e]); // check for IE mouseup when moving into the document again
			if (!this.opos || (_IE && !e.button && parseFloat(_IE_VERSION)<9)) return this.stop.apply(this, [e]); // check for IE mouseup when moving into the document again
			/* modified by liusp end 3. */
			var o = this.options;
